...
import { I18n, Utils, Loader, SelectID as DialogSelectID, GenericApp } from '@iobroker/adapter-react-v5';
...
class App extends GenericApp {
    constructor(props) {
        ...
        this.state = {
            ...this.state,
            isDrawOpen:
                window.localStorage.getItem('iobroker.scheduler.isDrawOpen') === null
                    ? true
    ...
    renderPriority(currentProfile) {
        return (
            <div
                className="mt-sm-auto mb-sm-auto"
                key="prioprity"
            >
                <div>
                    <PriorityPanel
                ...
                <DialogContent>
                    ...
                    <div style={{ width: '100%' }}>
                        <TextField
                            ...
                            InputProps={{
                                endAdornment: this.state.optionsDialog.holidayId ? (
                                    <InputAdornment position="end">
                                        <IconButton
                                            onClick={() => {
                                                const optionsDialog = JSON.parse(
                                                    JSON.stringify(this.state.optionsDialog),
                                                );
    ...
    render() {
        ...
        const content = (
            ...
            >
                {currentProfile ? (
                    <>
                        {this.state.isDrawOpen && !isMobile ? null : (
                            <div
                                style={{
                                    display: 'flex',
                                    alignItems: 'center',
